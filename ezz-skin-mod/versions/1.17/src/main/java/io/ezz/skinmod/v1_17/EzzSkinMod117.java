...
public class EzzSkinMod117 implements ClientModInitializer {
    ...
    private static synchronized void loadTexture() {
        ...
        if (!skinFile.exists()) {
            System.err.println("[EzzSkinMod-1.17] Skin file not found at " + skinFile.getAbsolutePath());
            return;
        }
        ...
        try (InputStream is = new FileInputStream(skinFile)) {
            ...
            System.out.println("[EzzSkinMod-1.17] Successfully registered custom skin texture: " + id);
        } catch (Exception e) {
            System.err.println("[EzzSkinMod-1.17] Error registering skin texture: " + e.getMessage());
        }
    }
}
